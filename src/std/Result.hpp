...
struct ResultStorage {
  ...
  ResultStorage(ResultStorage<T, E> &&other) : isValue(other.isValue) {
    if (isValue) {
...
struct Result {
  ...
  constexpr explicit Result(const Result<T, E> &other) : storage(other.storage) {}
  constexpr Result(T &&other) noexcept : storage(std::move(other)) {}
  constexpr Result(E &&other) noexcept : storage(std::move(other)) {}
  constexpr explicit Result(Result<T, E> &&other) noexcept
  ...
  const E &unwrapErr() const noexcept {
    DCHECK(isErr());
    return storage.error;
  }
};
...
template <typename T, typename E>
Result<T, E> flatten(const Result<Result<T, E>, E> &res) {
  if (res.isErr()) {
    return res.unwrapErr();
  }

  const Result<T, E> &inner = res.unwrap();
  if (inner.isErr()) {
